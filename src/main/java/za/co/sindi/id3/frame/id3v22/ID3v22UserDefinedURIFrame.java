 ...
 *
 */
public class ID3v22UserDefinedURIFrame extends ID3v22Frame implements ID3v2DescriptiveInformationFrame<URI> {

	private static final Logger logger = Logger.getLogger(ID3v22UserDefinedURIFrame.class);
	 ...
	 * @param id
	 */
	public ID3v22UserDefinedURIFrame(String id) {
		super(id);
	...
	public void decodeFrameData(byte[] data) throws ID3Exception {
		...
		try {
			...
			if (encoding == null) {
				throw new ID3Exception(ID3ExceptionStrings.generateInvalidEncodingFormat(2, data[0] & 0xFF));
			}
